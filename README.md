<h1 align="center">cofferdam</h1>

<p align="center"><em>A watertight enclosure for high-volume DevOps in the age of agents.</em></p>

---

A **cofferdam** is the temporary watertight wall engineers build in the middle of a river so they can pump out the water and work on dry ground. The flood is still all around you, held back by the wall. You just carved out a safe place to operate.

That is the problem with agent-driven operations. When a fleet of agents is generating infrastructure changes faster than any human can read them, the bottleneck stops being *authoring* changes and becomes *governing* them. The flood is real and it is rising. `cofferdam` is the wall: agents press their changes against it, and a single controlled gate decides what reaches production, in what order, and under whose authority.
...
|-------|--------------|--------------|
| **Admission** | [`admission`](src/admission.rs) | A per-agent token bucket caps how fast any one agent can push, so a single runaway loop cannot starve the fleet. A bounded priority queue orders what is waiting (highest priority first, FIFO within a priority) and applies backpressure once the enclosure is full. |
...
The unit that flows through all of it is an [`Intent`](src/intent.rs): a change an agent *wants* to make, fully attributed, tagged with how urgent it is (`Priority`) and how much it can break (`BlastRadius`). Agents never touch production directly. They submit intents. The cofferdam decides.
...
use cofferdam::policy::{BlastNeedsPriority, NoGlobalDestroy, ResourceAllowlist};
...
```rust
use cofferdam::intent::Intent;
use cofferdam::policy::{Policy, Verdict};
...
cofferdam demo - 4000 intents flung at the wall over 200 ticks
...
| v   | Surface                                                                 | Status |
|-----|-------------------------------------------------------------------------|--------|
| 0.1 | Intent model, per-agent token-bucket admission + bounded priority queue, deny-overrides policy gate, hash-chained ledger, end-to-end `Cofferdam` | **shipped** |
